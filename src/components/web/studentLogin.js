...
class StudentLogin extends Component {
  ...
  login(e) {
    ...
    firebase.database().ref('/ubitportal/student/' + email + '/').once('value').then((snapshot) => {
      console.log(snapshot.val());
      if (snapshot.val() !== null && snapshot.val().password === this.state.password) {
        console.log('password correct');
        // alert('Login Successfully');
        toast.notify(<div><Alert style={{backgroundColor: '#d4edda', color:'black'}}> student login successfully</Alert></div>,{
          duration: 2000,
          position:'top-right',
